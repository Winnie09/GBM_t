### GBM CD4 T cell
dm = read.csv('/home-4/whou10@jhu.edu/scratch/Wenpin/GBM_t/data/chris/cd4_diffusion.csv')
...
selectcell <- rownames(meta[meta$active.ident %in% c('CD4 CM', 'CD4 Naive', 'TREG', 'CD4 EM', 'CD4 EX'), ])
dm = dm[selectcell, ]
library(ggplot2)
...
saveRDS(psn, '/home-4/whou10@jhu.edu/scratch/Wenpin/GBM_t/data/order/CD4EM_CD4EX_pseudotime.rds')

#####
id = which(clu %in% c(1,5))
newclu <- clu[id]
n <- names(newclu)
newclu <- as.numeric(as.factor(newclu))
names(newclu) <- n
...
ggplot(data.frame(x = dm[ord,1], y = dm[ord,2], t = seq(1, length(ord)))) + 
  ...
  theme_classic() + xlab('DM1') + ylab('DM2')   ## useful
psn = data.frame(Cell = ord, Pseudotime = 1:length(ord), Celltype = as.character(active.ident)[match(ord, names(active.ident))], stringsAsFactors = FALSE)
saveRDS(psn, '/home-4/whou10@jhu.edu/scratch/Wenpin/GBM_t/data/order/CD4CM_TREG_pseudotime.rds')
